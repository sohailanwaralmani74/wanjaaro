...
(function() {
  ...
  window.calculate = function() {
    ...
    if (P <= 0 || n <= 0) {
      ...
      if (numPayments) numPayments.textContent = '—';
    ...
    var result = calculateAmortization(P, r, n);
    ...
    if (numPayments) numPayments.textContent = result.schedule.length + ' mo';

    generateInsights(result, P, r, n);
  ...
  // ── Generate Insights ──
  function generateInsights(result, P, r, n) {
    ...
    if (result.schedule.length > 0) {
      ...
      if (firstPayment) {
        var principalPct = (firstPayment.principal / firstPayment.payment) * 100;
        insights.push({
          icon: '📈',
          text: 'First payment: <strong>' + formatPercent(principalPct) + '</strong> goes to principal'
        });
      }

      if (lastPayment && lastPayment !== firstPayment) {
        var lastPrincipalPct = (lastPayment.principal / lastPayment.payment) * 100;
        insights.push({
          icon: '📉',
          text: 'Last payment: <strong>' + formatPercent(lastPrincipalPct) + '</strong> goes to principal'
        });
      }
    }

    if (result.schedule.length < n * 12 && result.schedule.length > 0) {
      var actualMonths = result.schedule.length;
      var originalMonths = n * 12;
      insights.push({
        icon: '⏱️',
        text: 'Loan paid in <strong>' + actualMonths + ' months</strong> (vs ' + originalMonths + ' months)'
      });
    }
    ...
    if (insights.length === 0) {
      insights.push({
        icon: '💡',
        text: 'Enter loan details to see your amortization schedule'
      });
    }
  ...
  window.copyResult = function() {
    ...
    var payments = numPayments ? numPayments.textContent : '';
    var text = 'Amortization Result:\nMonthly Payment: ' + monthly + '\nTotal Interest: ' + interest + '\nTotal Payment: ' + total + '\nNumber of Payments: ' + payments;
    if (navigator.clipboard) {
  ...
  window.exportChart = function() {
    ...
    link.download = 'amortization-chart.png';
...
if (typeof window.calculate === 'undefined') {
  window.calculate = function() {
    console.warn('Amortization Calculator not loaded yet.');
  };
}