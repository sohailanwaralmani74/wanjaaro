// ============================================================
// CREDIT CARD PAYOFF CALCULATOR (Single Plan)
// ============================================================
...
(function() {
  ...
  // ── Core Calculation ──
  function calculateCreditCardPayoff(P, r, payment) {
  ...
  window.calculate = function() {
    var P = parseFloat(balance ? balance.value : 0) || 0;
    ...
    var result = calculateCreditCardPayoff(P, r, payment);

    if (result.impossible) {
      if (payoffTime) payoffTime.textContent = 'Never';
      if (totalInterest) totalInterest.textContent = '∞';
      if (totalPayment) totalPayment.textContent = '∞';
  ...
  function generateInsights(result, P, r, payment) {
    ...
    if (result.months > 0 && result.months < 12) {
      insights.push({
        icon: '🎉',
        text: 'You\'ll be debt free in less than a year!'
      });
    } else if (result.months > 60) {
      insights.push({
        icon: '💡',
        text: 'Consider increasing your monthly payment to reduce interest'
      });
    }
    ...
    if (insights.length === 0) {
      insights.push({
        icon: '💡',
        text: 'Enter your credit card details to see your payoff plan'
      });
    }
  ...
  function updateChart(result) {
    ...
    if (currentChartType === 'line') {
      ...
      var totalPrincipal = parseFloat(balance ? balance.value : 0) || 0;
      ...
      var totalPrincipal2 = parseFloat(balance ? balance.value : 0) || 0;
  ...
  function updateShareURL() {
    ...
    if (balance) params.set('b', balance.value);
  ...
  window.copyResult = function() {
    ...
    var text = 'Credit Card Payoff Result:\nTime: ' + time + '\nTotal Interest: ' + interest + '\nTotal Payment: ' + total + '\nPayoff Date: ' + date;
    if (navigator.clipboard) {
  ...
  window.exportChart = function() {
    ...
    link.download = 'credit-card-payoff-chart.png';
  ...
  window.resetForm = function() {
    ...
    var defaultRate = 22;
    var defaultPayment = 200;

    if (balance) { balance.value = defaultBalance; if (balanceSlider) balanceSlider.value = defaultBalance; }
    ...
    updateSliderLabel('balance', defaultBalance);
  ...
  function loadFromURL() {
    ...
    if (params.has('b')) {
      var bVal = parseFloat(params.get('b'));
      if (!isNaN(bVal) && bVal >= 0) {
        if (balance) { balance.value = bVal; if (balanceSlider) balanceSlider.value = bVal; }
        updateSliderLabel('balance', bVal);
      }
    }
    if (params.has('r')) {
...
if (typeof window.calculate === 'undefined') {
  window.calculate = function() {
    console.warn('Credit Card Payoff Calculator not loaded yet.');
  };
}