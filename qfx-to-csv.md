---
layout: main
title: "QFX to CSV Converter – Free, Secure & Browser‑Based | DataFrog"
description: "Convert QFX (OFX) files to CSV instantly. No upload, no sign‑up. Fully client‑side. Download as .csv."
keywords: "qfx to csv, convert qfx to csv, qfx csv converter, ofx to csv, quicken to csv, qfx parser"
category: qfxFinance
---
...
[
  {
    ...
    "@type": "WebApplication",
    "@id": "https://datafrog.tools/qfx-to-csv#webapp",
    "name": "QFX to CSV Converter",
    "url": "https://datafrog.tools/qfx-to-csv",
    "description": "Convert QFX (OFX) files to CSV format. All processing stays in your browser.",
  ...
  },
  {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    "@id": "https://datafrog.tools/qfx-to-csv#faq",
    "mainEntity": [
      {
        "@type": "Question",
        "name": "What is QFX?",
  ...
  },
  {
    "@context": "https://schema.org",
    "@type": "HowTo",
    "@id": "https://datafrog.tools/qfx-to-csv#howto",
    "name": "How to Convert QFX to CSV",
    "description": "Step‑by‑step guide to convert your QFX file to CSV.",
    "tool": {
      "@type": "HowToTool",
      "name": "QFX to CSV Converter"
    },
...
<div id="qfx-hero" class="home-hero" >
  <h1>QFX to CSV Converter – Turn OFX Data into Spreadsheet‑Ready CSV</h1>
  <p>
    Convert QFX (OFX) files to CSV format instantly.
    No upload, no sign‑up – 100% browser‑based. Perfect for analysing Quicken, bank, or investment transactions in Excel, Google Sheets, or any spreadsheet tool.
  </p>
...
<!-- TOOL -->
<section aria-label="QFX to CSV Converter" style="display:flex;justify-content:center">
  ...
  <div class="panel-card" id="convert-panel" style="display:none;">
    <div class="panel-header">
      <div>
        ...
        <div class="panel-sub">Your QFX file is ready – click Convert to generate CSV</div>
      </div>
      <div class="btn-row">
...
<article class="onpage-content">
 <div class="blog-post-meta">
  ...
  <section id="how-it-works"><h2>How to convert – 3 simple steps</h2><ul><li>Upload your .qfx file</li><li>Click Convert</li><li>Copy or download the CSV</li></ul></section>
...
(function() {
  ...
  function parseQFX(xmlText) {
    ...
    if (stmttrns.length === 0) {
      // Try alternative: look for INVSTMTTRN for investment
      stmttrns = xmlDoc.getElementsByTagName('INVSTMTTRN');
    }
    if (stmttrns.length === 0) {
      // Maybe it's a credit card? Try CCSTMTTRN
      stmttrns = xmlDoc.getElementsByTagName('CCSTMTTRN');
    }

    // If still none, try to get all transaction nodes by path
    if (stmttrns.length === 0) {
      // Fallback: use XPath or generic search
      var nodes = xmlDoc.getElementsByTagName('STMTTRN');
      // We already did that.
      // Maybe it's in a different namespace? We'll just search all elements with TRNTYPE
      var allNodes = xmlDoc.getElementsByTagName('*');
      var possibleTrns = [];
      for (var i = 0; i < allNodes.length; i++) {
        if (allNodes[i].tagName === 'STMTTRN' || allNodes[i].tagName === 'INVSTMTTRN' || allNodes[i].tagName === 'CCSTMTTRN') {
          possibleTrns.push(allNodes[i]);
        }
      }
      if (possibleTrns.length > 0) {
        stmttrns = possibleTrns;
      } else {
        // We may need to recursively search: use getElementsByTagName for all depths.
        // Let's try with a different approach: extract all <STMTTRN> tags using regex fallback if needed
        // But we can trust DOMParser.
        // If still none, return empty.
      }
    }

    // If still no transactions, try to parse using regex as last resort (some QFX files are not well-formed XML)
    if (stmttrns.length === 0) {
      // Use regex to extract transaction blocks between <STMTTRN> and </STMTTRN>
      var blocks = xmlText.match(/<STMTTRN>([\s\S]*?)<\/STMTTRN>/g);
      if (blocks) {
        blocks.forEach(function(block) {
          var tx = {};
          var date = block.match(/<DTPOSTED>([^<]*)<\/DTPOSTED>/);
          if (date) tx.date = date[1].substring(0,8); // YYYYMMDD
    ...
    for (var i = 0; i < stmttrns.length; i++) {
      ...
      if (dateNode) {
        ...
        // Try DTSETTLEMENT for investment
      ...
      if (amtNode) tx.amount = parseFloat(amtNode.textContent) || 0;
      // Type
  ...
  function convertFile() {
    ...
    setTimeout(function() {
      try {
        ...
        if (outputLabel) outputLabel.textContent = 'CSV – ' + transactions.length + ' transactions';
